...
public class HinnakiriServiceImpl implements HinnakiriService {
	...
	throws HinnakiriNumberFormatException, HinnakiriNegativeException, HinnakiriZeroException, HinnakiriPrecisionException {
		...
		//negatiivse arvu kontroll
		if(maximumPrice < 0){
			throw new HinnakiriNegativeException();
		}
		
		//vordub nulliga kontroll
		if(maximumPrice == 0){
			throw new HinnakiriZeroException();
		}
		
		//tapsuse kontroll
		//muudab maximumPrice'i stringiks
		//poolitab selle komakohast
		//loeb komakoha taha jaavad arvud
		//kui neid on rohkem kui 2, viskab exceptioni
		//used: http://stackoverflow.com/questions/21825862/validation-to-check-a-double-for-2-decimal-places
		String[] splitter = maximumPrice.toString().split("\\.");
		int decimalLength = splitter[1].length() -1;
		if (decimalLength > 1){
